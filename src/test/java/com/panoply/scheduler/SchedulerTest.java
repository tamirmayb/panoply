...
import com.panoply.scheduler.Utils.SingleJob;
import com.panoply.scheduler.schedule.Schedules;
import com.panoply.scheduler.report.SchedulerStats;
...
import static com.panoply.scheduler.Utils.doNothing;
...
public class SchedulerTest {
	...
	public void check_that_two_job_cannot_be_scheduled_with_the_same_name() {
		...
		scheduler.schedule("job", doNothing(), Utils.oneMillsDelay());
		try {
			scheduler.schedule("job", doNothing(), Utils.oneMillsDelay());
			fail();
	...
	public void should_run_a_single_job() throws InterruptedException {
		...
		Utils.waitForJob(singleJob, () -> singleJob.countExecuted.get() > 0, 10000);
	...
	public void should_run_each_job_once() throws InterruptedException {
		...
		Thread thread1 = new Thread(() -> {
			Utils.waitForJob(job1, () -> job1.countExecuted.get() > 0, 10000);
		});
		thread1.start();
		Thread thread2 = new Thread(() -> {
			Utils.waitForJob(job2, () -> job2.countExecuted.get() > 0, 10000);
		});
		thread2.start();
		Thread thread3 = new Thread(() -> {
			Utils.waitForJob(job3, () -> job3.countExecuted.get() > 0, 10000);
		});
		thread3.start();
	...
	public void should_not_execute_past_job() throws InterruptedException {
		...
		Thread thread1 = new Thread(() -> {
			Utils.waitForJob(job1, () -> job1.countExecuted.get() > 0, 500);
		});
		thread1.start();
		thread1.join();
		scheduler.gracefullyShutdown();
	...
	public void exception_in_schedule_should_not_alter_scheduler() throws InterruptedException {
		...
		Thread thread1 = new Thread(() -> {
			Utils.waitForJob(job1, isJob1ExecutedAfterJob2::get, 10000);
		});
		thread1.start();
		thread1.join();
		Thread thread2 = new Thread(() -> {
	...
	public void check_that_a_scheduled_job_has_the_right_status() {
		...
		Job job = scheduler.schedule(doNothing(), Schedules.fixedDelaySchedule(Duration.ofSeconds(1)));
	...
	public void check_that_a_long_running_job_does_not_prevent_other_jobs_to_run() throws InterruptedException {
		...
		Job job = scheduler.schedule(doNothing(), Schedules.fixedDelaySchedule(Duration.ofMillis(10)));
		Thread.sleep(25L);
	...
	public void cancel_should_returned_a_job_with_the_done_status() throws Exception {
		Scheduler scheduler = new Scheduler(SchedulerConfig.builder().maxThreads(1).build());
		scheduler.schedule("jobToCancel", doNothing(), Schedules.fixedDelaySchedule(Duration.ofMillis(100)));
		Job job = scheduler.cancelJob("jobToCancel").toCompletableFuture().get(1, TimeUnit.SECONDS);

		assertThat(job).isNotNull();
		assertThat(job.status()).isEqualTo(JobStatus.DONE);
		assertThat(scheduler.jobStatus().size()).isEqualTo(1);
		assertThat(job.name()).isEqualTo("jobToCancel");
